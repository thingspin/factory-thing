...
#define INOUTARGID  60003
    ...
    UA_ObjectAttributes object_lepton;
    UA_ObjectAttributes_init(&object_lepton);
    object_lepton.description = UA_LOCALIZEDTEXT("en_US","Lepton");
    object_lepton.displayName = UA_LOCALIZEDTEXT("en_US","Lepton");
    UA_Server_addObjectNode(server, UA_NODEID_NUMERIC(1, LEPTONID),
                            UA_NODEID_NUMERIC(0, UA_NS0ID_OBJECTSFOLDER),
                            UA_NODEID_NUMERIC(0, UA_NS0ID_ORGANIZES), UA_QUALIFIEDNAME(1, "Lepton"),
                            UA_NODEID_NUMERIC(0, UA_NS0ID_FOLDERTYPE), object_lepton, NULL, NULL);
#define IMGSIZE 4096
	id = SNAPSHOTSID;
	UA_UInt32 type = UA_TYPES_BYTE;
	...
	UA_Variant_setArray(&attr.value, UA_Array_new((IMGSIZE), &UA_TYPES[type]), (IMGSIZE), &UA_TYPES[type]);
	UA_Server_addVariableNode(server, UA_NODEID_NUMERIC(1, id),
			UA_NODEID_NUMERIC(1, LEPTONID), 
			UA_NODEID_NUMERIC(0, UA_NS0ID_ORGANIZES), 
			qualifiedName, 
			UA_NODEID_NULL, attr, NULL, NULL);
	UA_Variant_deleteMembers(&attr.value);
	
    object_lepton.description = UA_LOCALIZEDTEXT("en_US","Boxes");
    object_lepton.displayName = UA_LOCALIZEDTEXT("en_US","Boxes");
    UA_Server_addObjectNode(server, UA_NODEID_NUMERIC(1, BOXESID),
                            UA_NODEID_NUMERIC(1, LEPTONID), UA_NODEID_NUMERIC(0, UA_NS0ID_ORGANIZES),
                            UA_QUALIFIEDNAME(1, "Boxes"),
                            UA_NODEID_NUMERIC(0, UA_NS0ID_FOLDERTYPE), object_lepton, NULL, NULL);
		
    object_lepton.description = UA_LOCALIZEDTEXT("en_US","Spots");
    object_lepton.displayName = UA_LOCALIZEDTEXT("en_US","Spots");
    UA_Server_addObjectNode(server, UA_NODEID_NUMERIC(1, SPOTSID),
                            UA_NODEID_NUMERIC(1, LEPTONID), 
							UA_NODEID_NUMERIC(0, UA_NS0ID_ORGANIZES),
                            UA_QUALIFIEDNAME(1, "Spots"),
                            UA_NODEID_NUMERIC(0, UA_NS0ID_FOLDERTYPE), object_lepton, NULL, NULL);
							

	UA_DataSource leptonHandler = (UA_DataSource) {.handle = NULL, .read = readLepton, .write = NULL};
	UA_NodeId sid;

    id = BOXESID;
    for(int i = 1; i < 5; i++) {
		
		int pid = BOXESID + (i*100);
		...
		object_attr.description = UA_LOCALIZEDTEXT("en_US", name);
		object_attr.displayName = UA_LOCALIZEDTEXT("en_US", name);
		UA_Server_addObjectNode(server, UA_NODEID_NUMERIC(1, pid),
								UA_NODEID_NUMERIC(1, BOXESID), 
								UA_NODEID_NUMERIC(0, UA_NS0ID_ORGANIZES),
								UA_QUALIFIEDNAME(1, name),
								UA_NODEID_NUMERIC(0, UA_NS0ID_FOLDERTYPE), object_attr, NULL, NULL);
        ...
        UA_Variant_setScalar(&attr.value, value, &UA_TYPES[type]);
        UA_Server_addDataSourceVariableNode(server, UA_NODEID_NUMERIC(1, pid+1),
                                  UA_NODEID_NUMERIC(1, pid),
                                  UA_NODEID_NUMERIC(0, UA_NS0ID_ORGANIZES),
                                  UA_QUALIFIEDNAME(1, name), UA_NODEID_NULL, attr, leptonHandler, &sid);
		UA_Variant_deleteMembers(&attr.value);
		
		type = UA_TYPES_FLOAT;
		value = UA_new(&UA_TYPES[type]);
        sprintf(name, "Max");
        attr.displayName = UA_LOCALIZEDTEXT("en_US", name);
				
        UA_Variant_setScalar(&attr.value, value, &UA_TYPES[type]);
        UA_Server_addDataSourceVariableNode(server, UA_NODEID_NUMERIC(1, pid+2),
                                  UA_NODEID_NUMERIC(1, pid),
                                  UA_NODEID_NUMERIC(0, UA_NS0ID_ORGANIZES),
                                  UA_QUALIFIEDNAME(1, name), UA_NODEID_NULL, attr, leptonHandler, &sid);
		UA_Variant_deleteMembers(&attr.value);
        ...
        sprintf(name, "Average");
		value = UA_new(&UA_TYPES[type]);
        attr.displayName = UA_LOCALIZEDTEXT("en_US", name);
				
        UA_Server_addDataSourceVariableNode(server, UA_NODEID_NUMERIC(1, pid+3),
                                  UA_NODEID_NUMERIC(1, pid),
                                  UA_NODEID_NUMERIC(0, UA_NS0ID_ORGANIZES),
                                  UA_QUALIFIEDNAME(1, name), UA_NODEID_NULL, attr, leptonHandler, &sid);
        ...
        attr.displayName = UA_LOCALIZEDTEXT("en_US", name);
        UA_Variant_setScalar(&attr.value, value, &UA_TYPES[type]);
        UA_Server_addDataSourceVariableNode(server, UA_NODEID_NUMERIC(1, pid+4),
                                  UA_NODEID_NUMERIC(1, pid),
                                  UA_NODEID_NUMERIC(0, UA_NS0ID_ORGANIZES),
                                  UA_QUALIFIEDNAME(1, name), UA_NODEID_NULL, attr, leptonHandler, &sid);
		UA_Variant_deleteMembers(&attr.value);
		...
		object_lepton.description = UA_LOCALIZEDTEXT("en_US", name);
		object_lepton.displayName = UA_LOCALIZEDTEXT("en_US", name);
		UA_Server_addObjectNode(server, UA_NODEID_NUMERIC(1, pid),
								UA_NODEID_NUMERIC(1, SPOTSID), 
								UA_NODEID_NUMERIC(0, UA_NS0ID_ORGANIZES),
								UA_QUALIFIEDNAME(1, name),
								UA_NODEID_NUMERIC(0, UA_NS0ID_FOLDERTYPE), object_lepton, NULL, NULL);
    ...
    // pthread_t tid = 0;
    // void* s = NULL;
	// int thread_handle = pthread_create(&tid, NULL, mqtt_pubsub, NULL);
